...
{
    // TO DO !!!
    cv::Vec3f homo_p(p[0],p[1],1);
    return cv::Vec3f(homo_p);
}
...
{
    // TO DO !!!
    cv::Vec2f euc_p(p[0]/p[2],p[1]/p[2]);
    return cv::Vec2f(euc_p);
}
...
cv::Vec3f getConnectingLine_2D(const cv::Vec3f& p1, const cv::Vec3f& p2)
{
    // TO DO !!!
    cv::Vec3f joint;
    joint[0]=p1[1]*p2[2]-p1[2]*p2[1];
    joint[1]=p1[2]*p2[0]-p1[0]*p2[2];
    joint[2]=p1[0]*p2[1]-p1[1]*p2[0];

    return cv::Vec3f(joint);
}
...
{
    // TO DO !!!
    cv::Matx33f T(1,0,dx,0,1,dy,0,0,1);
    return cv::Matx33f(T);
}
...
cv::Matx33f getRotationMatrix_2D(float phi)
{
    ...
    phi=phi*PI;
    phi=phi/180;
    cv::Matx33f R(cos(phi),-sin(phi),0,sin(phi),cos(phi),0,0,0,1);
    return cv::Matx33f(R);
}
...
{
    // TO DO !!!
    cv::Matx33f S(lambda,0,0,0,lambda,0,0,0,1);
    return cv::Matx33f(S);
}
...
std::vector<cv::Vec3f> applyH_2D(const std::vector<cv::Vec3f>& geomObjects, const cv::Matx33f &H, GeometryType type)
{
    ...
    switch (type) {
        ...
        while (i<size){
        // TO DO !!!
        cv::Vec3f p_item= H*geomObjects[i];
        result.push_back(p_item);
        i++;
        }
        } break;
        case GEOM_TYPE_LINE: {
            ...
            int i=0;
            while(i<size){
            cv::Vec3f l_item =(H.inv().t())*geomObjects[i];
            result.push_back(l_item);
            i++;
            }
        } break;
        default:
...
void run(const std::string &fname){
    ...
    cv::Vec3f v1(2.0f,3.0f,1.0f);
    // define v2 as homogeneous version of y
    cv::Vec3f v2(-4.0f,5.0f,1.0f);
